...
\copy dim_date (date_key, full_date, day_of_week, day_name, week_number, month, month_name, quarter, year) FROM '05_ETL/data/warehouse_ready/dim_date.csv' WITH (FORMAT csv, HEADER true);

\copy dim_account (account_key, account_id, account_name, account_type, industry, country, status) FROM '05_ETL/data/warehouse_ready/dim_account.csv' WITH (FORMAT csv, HEADER true);

\copy dim_customer (customer_key, customer_id, account_key, customer_name, customer_segment, industry, country, status) FROM '05_ETL/data/warehouse_ready/dim_customer.csv' WITH (FORMAT csv, HEADER true);

\copy dim_supplier (supplier_key, supplier_id, supplier_name, supplier_category, country, status) FROM '05_ETL/data/warehouse_ready/dim_supplier.csv' WITH (FORMAT csv, HEADER true);

\copy dim_product (product_key, product_id, supplier_key, product_name, category, subcategory, unit_of_measure, unit_cost, unit_price, status) FROM '05_ETL/data/warehouse_ready/dim_product.csv' WITH (FORMAT csv, HEADER true);

\copy dim_location (location_key, location_id, location_name, location_type, city, state_region, country, status) FROM '05_ETL/data/warehouse_ready/dim_location.csv' WITH (FORMAT csv, HEADER true);

\copy dim_employee (employee_key, employee_id, location_key, employee_name, department, role, hire_date, status) FROM '05_ETL/data/warehouse_ready/dim_employee.csv' WITH (FORMAT csv, HEADER true);

\copy dim_machine (machine_key, machine_id, location_key, machine_name, machine_type, installation_date, status) FROM '05_ETL/data/warehouse_ready/dim_machine.csv' WITH (FORMAT csv, HEADER true);
...
\copy fact_financial_transaction (financial_transaction_key, transaction_id, date_key, account_key, location_key, transaction_type, transaction_category, transaction_amount) FROM '05_ETL/data/warehouse_ready/fact_financial_transaction.csv' WITH (FORMAT csv, HEADER true);
